...
type
  ...
  /// <summary>
  /// Tiger Hash Algorithm 192 Bit (Tiger)
  /// </summary>
  THashTiger = class(TBlockHash)
  private
  ...
  protected
    ...
    property State: THashValue192 read FState;
    function PadFirstByte: Byte; virtual;
  public
...
{ THashTiger2 }
  ...
  THashTiger2 = class(THashTiger)
  protected
    function PadFirstByte: Byte; override;
  end;

implementation

const
  T1: array[Byte] of UInt64 = (
...
procedure TigerRound(var A, B, C: UInt64; const X, Mul: UInt64); inline;
begin
  C := C xor X;
  Dec(A,
    T1[C and $FF] xor
...
type
  TArray8UINT = array[0..7] of UInt64;

procedure TigerPass(var A, B, C: UInt64; const X: TArray8UINT; const Mul: UInt64); inline;
begin
...
procedure TigerKeySchedule(var X: TArray8UINT); inline;
begin
...
constructor THashTiger.Create(const APassCount: Word);
begin
  inherited Create;
...
class function THashTiger.HashType: THashType;
begin
  Result := THashType.Cryptographic;
end;

function THashTiger.HashSize: Cardinal;
begin
  Result := 24;
end;

function THashTiger.BlockSize: Cardinal;
begin
  Result := 64;
end;

procedure THashTiger.Initialize;
begin
  inherited;
  with FState do
  begin
...
procedure THashTiger.UpdateBlock(const Block: Pointer);
var
  A, B, C: UInt64;
  X: TArray8UINT;
  Tmp: UInt64;
...
begin
  ...
  B := FState.B;
  C := FState.C;
  Move(Block^, X, SizeOf(X));
  for Pass := 0 to PassCount - 1 do
  begin
    if Pass <> 0 then TigerKeySchedule(X);
    if Pass = 0 then
      TigerPass(A, B, C, X, 5)
    else if Pass = 1 then
      TigerPass(A, B, C, X, 7)
  ...
  FState.A := A xor FState.A;
  FState.B := B - FState.B;
  FState.C := C + FState.C;
end;

function THashTiger.GetPadBuffer: TBytes;
var
  LengthInBits: UInt64;
  i, PadLen: Word;
begin
  ...
  Result[0] := PadFirstByte;
  for i := 1 to PadLen - 1 do
    Result[i] := 0;
  PUInt64(@Result[PadLen])^ := LengthInBits;
end;

function THashTiger.PadFirstByte: Byte;
begin
  Result := $01;
end;

procedure THashTiger.Finalize;
begin
  inherited;
...
function THashTiger2.PadFirstByte: Byte;
begin
  Result := $80; // as in MD5 etc.
end;
