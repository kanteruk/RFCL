...
class function THashCKSum.HashType: THashType;
begin
  Result := THashType.Checksum;
end;

function THashCKSum.HashSize: Cardinal;
begin
  Result := 4;
end;

procedure THashCKSum.Initialize;
begin
  FContext := 0;
  FLength := 0;
end;
...
begin
  ...
  for i := 0 to Size - 1 do
    tmp := (tmp shl 8) xor CKSumTable[((tmp shr 24) xor PByteArray(Buffer)[i]) and $FF];
  FContext := tmp;
  Inc(FLength, Size);
end;
...
begin
  ...
  while L <> 0 do
  begin
    tmp := (tmp shl 8) xor CKSumTable[((tmp shr 24) xor (L and $FF)) and $FF];
    L := L shr 8;
  end;
  FContext := not tmp and $FFFFFFFF;
...
class function THashCKSumMPEG2.HashType: THashType;
begin
  Result := THashType.Checksum;
end;

function THashCKSumMPEG2.HashSize: Cardinal;
begin
  Result := 4;
end;

procedure THashCKSumMPEG2.Initialize;
begin
  FContext := $FFFFFFFF;
end;
...
procedure THashCKSumMPEG2.Finalize;
begin
	FContext := FContext and $FFFFFFFF;
  with LongRec(FContext) do
  begin
    FValue[0] := Bytes[3];
    FValue[1] := Bytes[2];
